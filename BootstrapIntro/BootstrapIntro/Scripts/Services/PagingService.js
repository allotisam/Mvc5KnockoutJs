function PagingService(resultList) {
    ...
    self.fetchEntities = function (event) {
        ...
        $.ajax({
            dataType: 'json',
            url: url,
            success: function (data) { self.updateResultList(data); },
            error: function () { $('.body-content').prepend(`<div class="alert alert-danger"><strong>Error!</strong> There was an error fetching the data.</div>`); }
        });
    };
